...
# Add support for checking supported file types
...
class AgenticRAGCLI:
    ...
    def print_help(self):
        ...
        try:
            ...
            file_types_text = ", ".join(supported_extensions)
        except Exception:
            file_types_text = ".md, .txt, .xlsx, .docx, .pdf, .jpg, .png, etc."
...
{Colors.BOLD}Available Commands:{Colors.END}
  ...
  {Colors.GREEN}files{Colors.END}          - Show supported file types and current documents
...
{Colors.BOLD}Supported File Types:{Colors.END}
  {file_types_text}
...
{Colors.BOLD}Examples:{Colors.END}
  ...
  - "What's in the Excel file about GNP?"
"""
        print(help_text)
    ...
    async def show_files(self):
        """Show supported file types and current documents."""
        try:
            processor = create_document_processor()
            supported_extensions = processor.get_supported_extensions()
            ...
            for ext in supported_extensions:
                print(f"  {Colors.GREEN}{ext}{Colors.END}")
            
            # Check for documents in the Documents folder
            documents_dir = Path("Documents")
            if documents_dir.exists():
                print(f"\n{Colors.BOLD}Current Documents:{Colors.END}")
                ...
                if supported_files:
                    for file_path in sorted(supported_files):
                        ...
                        print(f"  {Colors.CYAN}{file_path.name}{Colors.END} "
                              f"({Colors.YELLOW}{file_type}{Colors.END}, {size_str})")
                else:
                    print(f"  {Colors.YELLOW}No supported documents found{Colors.END}")
            else:
                print(f"\n{Colors.YELLOW}Documents folder not found{Colors.END}")
            
            print()
            
        except Exception as e:
            print(f"{Colors.RED}✗ Error checking files: {e}{Colors.END}")
